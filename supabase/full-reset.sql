-- ============================================================
-- RecoverPath — FULL DATABASE RESET (Drop + Create + Seed)
-- ============================================================
-- Paste this ENTIRE script into the Supabase SQL Editor and Run.
-- It will drop all existing tables and recreate everything fresh.
-- ============================================================

-- ── DROP EXISTING TABLES ─────────────────────────────────────
DROP TABLE IF EXISTS "MedicationLog" CASCADE;
DROP TABLE IF EXISTS "VitalLog" CASCADE;
DROP TABLE IF EXISTS "Medication" CASCADE;
DROP TABLE IF EXISTS "DischargePlan" CASCADE;
...
CREATE TABLE "DischargePlan" (
    ...
    "patientId"            TEXT NOT NULL REFERENCES "Patient"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "hospitalId"           TEXT NOT NULL REFERENCES "Hospital"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "createdAt"            TIMESTAMPTZ NOT NULL DEFAULT now(),
    "updatedAt"            TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE "Medication" (
    ...
    "endDate"         TIMESTAMPTZ,
    "dischargePlanId" TEXT REFERENCES "DischargePlan"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "patientId"       TEXT REFERENCES "Patient"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "createdAt"       TIMESTAMPTZ NOT NULL DEFAULT now(),
    "updatedAt"       TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE "MedicationLog" (
    ...
    "notes"        TEXT,
    "patientId"    TEXT NOT NULL REFERENCES "Patient"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "medicationId" TEXT NOT NULL REFERENCES "Medication"("id") ON DELETE CASCADE ON UPDATE CASCADE,
    "createdAt"    TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX "DischargePlan_patientId_idx" ON "DischargePlan"("patientId");
CREATE INDEX "DischargePlan_hospitalId_idx" ON "DischargePlan"("hospitalId");
CREATE INDEX "Medication_dischargePlanId_idx" ON "Medication"("dischargePlanId");
CREATE INDEX "Medication_patientId_idx" ON "Medication"("patientId");
CREATE INDEX "VitalLog_patientId_idx" ON "VitalLog"("patientId");
CREATE INDEX "VitalLog_date_idx" ON "VitalLog"("date");
...
-- VITAL LOGS (random sample over last 14 days)
...
SELECT
    ...
    now()
FROM
    (VALUES ('pat_arjun'),('pat_sneha'),('pat_rahul'),('pat_priya'),('pat_amit'),
            ('pat_kavita'),('pat_vikash'),('pat_ananya'),('pat_deepak'),('pat_ritu')) AS p(pid),
